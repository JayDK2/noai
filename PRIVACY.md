...
### If you turn on Content Credentials

This one feature is off unless you switch it on, and switching it on asks your
browser for permission to run on every site. Here is exactly what it then does.

For images large enough to be worth checking, NoAI reads the first 256 KB of the
image file and looks for a **Content Credential** — a declaration that some
tools (Adobe, OpenAI, Google, Leica and others) write into the file itself. If
that declaration says the image was generated by AI, NoAI marks it.

- The read is made with the browser's cache preferred, so in most cases it costs
  no additional network request — the image is already on your machine.
- When the cache does not have it, your browser requests part of that image from
  the site it is on. That site can see the request, exactly as it saw the request
  your browser already made to display the image. Nothing extra about you is sent.
- Results are remembered only in memory, only for the current browser session,
  and only as "this image URL declared AI / did not".
- No image, no URL, no page address and nothing else is sent to us or to anyone
  else. There is no server involved.

NoAI reads the declaration; it does not verify the cryptographic signature.
The mark therefore says *declared by the file* — not *proven*.
